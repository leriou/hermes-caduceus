...
import { RefreshCw, Activity, Server, AlertTriangle, Zap, Clock } from "lucide-react";
import type { LucideIcon } from "lucide-react";
import PluginMetricsPanel from "../../components/PluginMetricsPanel";
...
function Monitoring({ profile }: MonitoringProps): React.JSX.Element {
  const { t } = useI18n();
  ...
  return (
    <div className="home-container">
      <div className="home-header">
        <div className="home-title-row"><Activity size={18} /><h2 className="home-title">{t("navigation.monitoring")}</h2></div>
      ...
      {/* Latest error */}
      {health?.errors.latestSummary && (
        <div className="home-section">
          <h3 className="home-section-title">Latest Error</h3>
          <div className="home-latest-error"><code>{health.errors.latestSummary}</code></div>
        </div>
      )}

      {/* Plugin metrics */}
      <PluginMetricsPanel profile={profile} />
    </div>
  );
}
